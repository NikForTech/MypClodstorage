...
function validateSecretKey(req, res, next) {
  ...
  if (!expected) return res.status(500).json({ success: false, message: 'Server configuration error' });
...
async function uploadToCloudinaryPool(fileBuffer, originalFilename) {
  if (cloudinaryAccounts.length === 0) throw new Error('No Cloudinary accounts configured');
  ...
  for (let i = 0; i < cloudinaryAccounts.length; i++) {
    ...
    try {
      ...
      return { url: result.secure_url, id: result.public_id, service: account.name };
...
// Health check
app.get('/', (req, res) => {
  res.json({ success: true, message: 'NFT Upload API is running 🚀' });
});
